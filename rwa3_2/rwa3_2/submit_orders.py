...
class OrderSubmission():
    ...
    def Submit_Order(self, agv_id, order_id):
        ...
        if self.agv_id not in self.subscribers:
            ...
            self.agv_status_subscriber = self.node.create_subscription(AGVStatus, '/ariac/agv{}_status'.format(self.agv_id), self.agv_status_callback)
    ...
    def call_submit_order_service(self):
        ...
        future.add_done_callback(self.order_submission_callback)
    ...
    def order_submission_callback(self, future):
        try:
            ...
            if response.success:
                ...
                return True  # Return True if order submission is successful
            else:
                self.node.get_logger().error("Order submission failed: {}".format(response.message))
                return False  # Return False if order submission fails
